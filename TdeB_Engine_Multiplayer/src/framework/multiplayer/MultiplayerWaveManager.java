...
public class MultiplayerWaveManager {
	...
	public MultiplayerWaveManager(MultiplayerServer server) {
		this.server = server;
	...
	public void update() {
		...
		if(System.currentTimeMillis() - timer > waveOffsetInMSEC && !waveHasSpawned && (System.currentTimeMillis() - lastEnemyDead > 2000)) {	
			...
			if(Collection.ARENA_CURRENT_WAVE <= 10) {
				totalEnemiesInCurrentWave = (((int)enemiesInWave) + 1) * server.getSpawnPoints().size() - 1;
			}else {
				totalEnemiesInCurrentWave = (((int)enemiesInWave) + 1) * server.getSpawnPoints().size();
			}
...
//			}
	...
	private void spawnWave() {
		...
		if(t1 - t2 > 2000 && currentEnemiesInWave <= enemiesInWave) {
			...
			for(Spawner point : server.getSpawnPoints()) {
				currentEnemiesInWave++;
				spawnCounter++;
				// use only 2 spawnpoints when wave < 10
				if(Collection.ARENA_CURRENT_WAVE <= 10 && spawnCounter == server.getSpawnPoints().size()) {
					spawnCounter = 0;
					break;
				}
				...
				if(waveCounter < 10) {
					if(random < 0.85) {
						Enemy_Spider tmp = new Enemy_Spider(point.getX(), point.getY(), TILE_SIZE, TILE_SIZE, server.getHandler());
						server.getEnemyList().add(tmp);
						shadowObstacleList.add(tmp);
					}else if(random >= 0.85) {
						Enemy_Fly tmp = new Enemy_Fly(point.getX(), point.getY(), TILE_SIZE, TILE_SIZE, server.getHandler());
						server.getEnemyList().add(tmp);
						shadowObstacleList.add(tmp);
					}
				...
				// WAVE >= 10 && < 20
					...
					if(random < 0.5) {
						...
						Enemy_Devil tmp = new Enemy_Devil(point.getX(), point.getY(), TILE_SIZE, TILE_SIZE, server.getHandler());
						server.getEnemyList().add(tmp);
						shadowObstacleList.add(tmp);
					}else if(random >= 0.7 && random < 0.95) {
						Enemy_Ghost tmp = new Enemy_Ghost(point.getX(), point.getY(), TILE_SIZE, TILE_SIZE, server.getHandler());
						server.getEnemyList().add(tmp);
						shadowObstacleList.add(tmp);
					}else if(random >= 0.95) {
						Node n = getRandomNode();
						Enemy_Digger tmp = new Enemy_Digger(n.getX() * TILE_SIZE, n.getY() * TILE_SIZE, TILE_SIZE, TILE_SIZE, server.getHandler());
						server.getEnemyList().add(tmp);
					}
	...
	private Node getRandomNode() {
		Node n = null;
		LinkedList<Node> nodes = server.getGraph().getNodes();
