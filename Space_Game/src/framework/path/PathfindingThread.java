		...
		{		
			...
			{		
				try {
					...
					if(graph.getNodeID(enemyX, enemyY) != -1 && graph.getNodeID((int)handler.getPlayer().getX()/TILE_SIZE, (int)handler.getPlayer().getY()/TILE_SIZE) != -1)
					{
						t1 = System.currentTimeMillis();
						if(t1 - t2 > 25 && e.getSpeed() != 0)
						{
							e.setPathLock(true);
							if(e != null){
								LinkedList<Node> tmp = graph.astar(graph.getNodeID(enemyX, enemyY), graph.getNodeID((int)(handler.getPlayer().getX())/TILE_SIZE, (int)(handler.getPlayer().getY())/TILE_SIZE));
